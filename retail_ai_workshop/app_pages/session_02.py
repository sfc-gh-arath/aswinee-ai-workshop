...
- **SALES_TRANSACTIONS**: The central fact table. Each row is a sale at a specific store or online. The `store_id` and `product_id` columns create relationships to our Session 1 dimension tables. The `transaction_date` distribution is intentionally skewed toward Q4 (holiday season) and August (back-to-school) to simulate real retail seasonality.

- **PURCHASE_ORDERS**: The supply chain backbone. Tracks orders placed with suppliers through delivery. The many-to-one relationship (multiple POs per supplier) reflects real procurement where Alpine & Co. places seasonal buy orders months in advance.

- **INVENTORY_LEVELS**: Point-in-time inventory snapshots per store per product. The `days_of_supply` metric is critical for retail operations - it tells merchants how many days of sales the current inventory can cover.

**Why this matters for AI/ML**: These tables provide the structured features we'll use in Session 4 for ML model training (predicting stockouts). The relationship between `quantity_on_hand`, `reorder_point`, and `days_of_supply` becomes our prediction signal.

**Data modeling pattern**: This follows a **star schema** design - SALES_TRANSACTIONS is the central fact table, with PRODUCTS, STORES, and SUPPLIERS as dimension tables. This is the most common pattern for analytical workloads in Snowflake.
""")
...
- **STORE_FOOT_TRAFFIC**: In-store traffic from people counters (sensors at store entrances). Visitor count combined with conversion rate tells you how effectively the store converts browsers into buyers. Weather data is included because it significantly impacts foot traffic in retail.

- **DAILY_SALES_METRICS**: Aggregated daily KPIs per store. This is the data that store managers review every morning. The returns metrics are important - high return rates on specific products signal quality or fit issues.

- **WEBSITE_CLICKSTREAM**: E-commerce browsing behavior. The `page_type` column represents the conversion funnel: home -> category -> product -> cart -> checkout. Drop-off at each stage reveals UX issues. We'll use this for customer behavior analysis in later sessions.

- **INVENTORY_SNAPSHOTS**: Time-stamped inventory positions. The `projected_stockout_date` is calculated from current velocity and on-hand quantity. Rows where stockout is projected within 7 days are urgent replenishment signals.
...
- **CUSTOMER_REVIEWS**: Product review text that we'll use for sentiment analysis, entity extraction, and summarization. Reviews contain rich signals: sizing feedback, quality complaints, competitor comparisons, and style opinions.

- **SUPPORT_TICKETS**: Customer service interactions. These contain free-form text that we'll classify by intent, extract key entities from, and use to identify product quality trends across the catalog.

- **MARKETING_CAMPAIGNS**: Campaign briefs and performance data. The text fields describe targeting strategy and creative direction - useful for AI-powered campaign analysis and optimization.

- **SUPPLIER_COMMUNICATIONS**: Includes **bilingual content** (English/Spanish) to demonstrate Cortex translation capabilities. International apparel suppliers in Vietnam, China, and Latin America often communicate in their local language.

- **PRODUCT_RETURN_NOTES**: Detailed return inspection narratives. These power root-cause analysis for returns - is it a sizing issue (product runs small), quality defect (seam failure), or customer preference (changed mind)? This distinction drives different business actions.

**Why text data matters for AI**: Traditional BI only works with structured data. Modern AI can extract insights from text, classify documents, answer questions from document collections, and detect sentiment - all capabilities we'll build in later sessions.
""")
...
render_domain_glossary([
    {"term": "SKU (Stock Keeping Unit)", "definition": "A unique identifier for each distinct product variant. A single style like 'Summit Performance Tee' in 5 sizes and 4 colors = 20 SKUs. Alpine & Co. manages thousands of active SKUs across branded and private-label products."},
    {"term": "BOPIS (Buy Online, Pick Up In Store)", "definition": "An omnichannel fulfillment method where customers purchase online and collect at a physical store. BOPIS drives foot traffic (customers often buy additional items during pickup) and reduces shipping costs."},
    {"term": "Sell-through Rate", "definition": "The percentage of inventory received that has been sold. Calculated as (units sold / units received) x 100. Healthy apparel sell-through is 60-70% at full price. Below 50% triggers markdowns."},
    ...
    {"term": "Markdown", "definition": "A permanent price reduction to clear slow-moving inventory. In apparel, markdowns typically start at 25-30% off and increase to 50-70% during end-of-season clearance. Excessive markdowns destroy margin."},
])
