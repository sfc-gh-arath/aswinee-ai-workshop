...
render_session_header(12, "Deploying AI Apps with Streamlit", "3:20 - 3:40 PM", "20 min", "3-page retail dashboard with chat interface")
...
render_technologies_used([
    ...
    {"name": "Compute Pool", "description": "A managed pool of container nodes that powers SiS apps on the container runtime. Provides CPU resources, auto-scales, and supports any Python package from pip or conda.", "icon": "memory"},
...
- KPI cards at the top showing: Total Revenue this month, Total Units Sold, Avg Transaction Value, Online Sales % (from SALES_TRANSACTIONS)
- A map of store locations (New York, Los Angeles, Chicago, Houston, Portland, Denver, Miami, Seattle) with markers sized by revenue. Use st.pydeck_chart with a ScatterplotLayer — hardcode the store lat/lon coordinates for the 8 Alpine & Co. locations
- A bar chart of revenue by product category
- A line chart showing daily sales over the past 90 days
...
- Shows top-selling products this week in a sidebar
- Has a dropdown to select which LLM model to use

PAGE 3 - Customer Insights:
- Summary cards: Total Reviews this month, Avg Rating, Open Support Tickets
- A table of recent SUPPORT_TICKETS with priority color coding (Critical = red, High = orange, Medium = yellow, Low = green)
...
```sql
COPY INTO @RETAIL_AI_DEMO.RETAIL_OPS.STREAMLIT_STAGE/pyproject.toml
FROM (SELECT '[project]
name = "retail-ai-dashboard"
version = "1.0.0"
requires-python = ">=3.11"
...
rev_df = session.sql("SELECT SUM(total_amount) FROM SALES_TRANSACTIONS WHERE ...").collect()
col1.metric("Total Revenue", f"${rev_df[0][0]:,.0f}")
```

**Page 2 - Chat interface** uses `st.chat_input` and `st.chat_message` with Cortex LLM functions.

**Page 3 - Customer Insights** with conditional priority styling for support tickets.

**Key SiS advantages**:
- **No data movement**: App runs inside Snowflake, queries data directly
- **Security**: Inherits the user's role and permissions (including masking policies from earlier sessions)
...
```
https://app.snowflake.com/<account>/#/streamlit-apps/RETAIL_AI_DEMO.RETAIL_OPS.RETAIL_DASHBOARD
```
...
Now the RETAIL_MERCHANDISER role can access the dashboard - but they'll see masked financial data (from earlier sessions' masking policies) and access will respect row-level and column-level security. The app automatically inherits Snowflake's security model.
""")
...
render_what_you_built([
    ...
    "RETAIL_DASHBOARD - 3-page Streamlit app on container runtime",
    "Sales Dashboard with KPIs, store map, charts, and stockout scores",
    "AI-powered Retail Intelligence chat interface with model selection",
    "Customer Insights page with support ticket tracking",
])
