...
PROMPT_13_1 = """In RETAIL_AI_DEMO.RETAIL_OPS, run these observability queries:
...
PROMPT_13_2 = """In RETAIL_AI_DEMO.RETAIL_OPS, analyze our workshop's AI cost footprint:

1. Calculate the total estimated cost breakdown of everything we built today:
   - Warehouse compute credits (RETAIL_AI_WH usage)
...
2. Generate recommendations for production optimization:
   ...
   - Whether to use dynamic tables vs scheduled tasks for the stockout scoring
...
PROMPT_13_3 = """In RETAIL_AI_DEMO.RETAIL_OPS, create a final summary of everything we built today:

1. List all objects in RETAIL_AI_DEMO.RETAIL_OPS with their types:
...
```sql
SHOW TABLES IN SCHEMA RETAIL_AI_DEMO.RETAIL_OPS;
SHOW VIEWS IN SCHEMA RETAIL_AI_DEMO.RETAIL_OPS;
...
SHOW STAGES IN SCHEMA RETAIL_AI_DEMO.RETAIL_OPS;
SHOW USER FUNCTIONS IN SCHEMA RETAIL_AI_DEMO.RETAIL_OPS;
```
...
- 1 semantic view (RETAIL_OPERATIONS_VIEW)
- 1 UDF (CALCULATE_STOCKOUT_RISK)
- 1 Agent (RETAIL_OPS_AGENT)
...
render_domain_glossary([
    {"term": "Full AI Pipeline (Workshop)", "definition": "The end-to-end architecture built across all 14 sessions: data ingestion → feature engineering → ML training → real-time scoring → customer feedback extraction → semantic search → RAG → Agent orchestration → app deployment → monitoring. Each component is a Snowflake-native object."},
    {"term": "Cost Profile of AI Workloads", "definition": "AI workloads have a different cost profile than traditional BI. LLM inference (COMPLETE) is expensive per call; sentiment/translate are cheaper. Understanding which tasks need which models is key to production cost management. For retail, high-volume tasks like product classification should use smaller models."},
])
